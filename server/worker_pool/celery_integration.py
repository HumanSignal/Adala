...
def worker_process_init_handler(**kwargs):
    ...
    def run_async_processors():
        ...
        async def start_processors():
            ...
            try:
                ...
                prediction_queue = asyncio.Queue(maxsize=100)  # Bounded queue for predictions
    ...
    logger.info(f"Worker and output processors started with shared queue for PID {os.getpid()}")
...
def worker_process_shutdown_handler(**kwargs):
    ...
    if processor_thread and processor_thread.is_alive():
        ...
        if processor_thread.is_alive():
            logger.warning(f"Processor thread did not shut down gracefully for PID {os.getpid()}")

...
from .processor import WorkMessage
...
async def submit_batch(request: SubmitBatchRequest) -> SubmitBatchResponse:
    ...
    try:
        ...
        await producer.send_and_wait(
            "worker_pool_input",
            value=work_message.__dict__
        )
        
        logger.info(f"Submitted batch {batch_id} to worker pool with API key: {'present' if request.api_key else 'missing'}")
